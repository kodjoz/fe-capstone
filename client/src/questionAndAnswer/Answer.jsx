...
const Answer = (props) => {
  ...
  const handleHelpfulOrReport = (endpoint) => {
    ...
    } else if (isReported && endpoint === 'report') {
      alert('You already reported this answer!');
    } else {
      props.markOrReport('answers', props.answer.answer_id, endpoint);
  ...
  let answer = props.answer ? props.answer : null;
  ...
  return (
    <AnswerWrapper>
      ...
      <AnswerLinks>by {answer.answerer_name}, <time>{prettyDate}</time> | <Helpful>Helpful? </Helpful> <HelpfulLinkPreset
        onClick={() => handleHelpfulOrReport('helpful')}
        underline={isHelpful ? 'none' : 'underline'}>Yes ({answer.helpfulness})</HelpfulLinkPreset> | <HelpfulLinkPreset
        onClick={() => this.handleHelpfulOrReport('report')}
        underline={isReported ? 'none' : 'underline'}>{!isReported ? 'Report' : 'Reported!'}</HelpfulLinkPreset></AnswerLinks>
    </AnswerWrapper>
  );
};
